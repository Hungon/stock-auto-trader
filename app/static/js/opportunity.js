...
function renderDetail(sig) {
  ...
  // Expose the selection for the AI Perspective module and reset its output.
  window.currentOpportunity = { market: sig.market, symbol: sig.symbol, name: sig.name };
  const aiOut = document.getElementById("ai-perspective-output");
  if (aiOut) {
